...
?>
			...
			<div class="content">
                ...
                if(isset($_GET['action'],$_GET['ids']) && $_GET['action']=='edit'){
                  $id=$_GET['ids'];
                  $sql="select * from sleeping_afternoon where id=?";
                  $stm=$con->prepare($sql); 
...
if(isset($_POST['save'])){
	function test_input($data) {
		...
		else
		{
			...
			$query="UPDATE  sleeping_afternoon SET branch=?,code=?,sender_name=?,date=?,exit_date=?,visites=?,medicin_m=?,hart_m=?,heat_m=?,breathe_m=?,inside_m=?,outside_m=?,medical=?,program=? where id=?";
			...
			if($stm->rowCount())	
			{
				...
				echo "<script>
					window.open('today_sleeping_afternoon_data.php?num=6&id=8','_self')
		 		 </script>";
			}
			else
			{
