...
if(isset($_POST['save'])){
	...
	if(	empty($_POST["violense"])){
		...
		else
		{
			...
			$query="UPDATE  doctor SET branch=?,psyshological_diagnosis=?,note=?,date=?,Physical_Examination=?,lap=?,medical=?,appraisal=?,quantity=?,diagnosis=? WHERE id=? ";
			$stm=$con->prepare($query);
			$stm->execute(array($branch,$psyshological_diagnosis,$note,$date,$Physical_Examination,$b,$medical,$appraisal,$quantity,$diagnosis,$id));
			if($stm->rowCount())	
			{
				echo '<script src="js/send_success.js"></script>';
				echo "<script>
                          window.open('today_doctor_data.php?num=5&id=6','_self')
                       </script>";
			}
			else
			{
