                                ...
                                <form action="" method="post" enctype="multipart/form-data">
                                    <div class="card">
                                        <div class="card-header">
                                            <div class="card-title"> اضافة تقييم اسبوعي للموظف</div>
                                        </div>
                                        <div class="card-body">
                                            <div class="row">
...
<?php
	if(isset($_POST['save'])){
        ...
        $sender_name=$name['name'];
            ...
            $sql="select * from week_evaluation_form where  emp=? and  date=?  ";
            $stm=$con->prepare($sql); 
            $stm->execute(array($emp,$today));
	        if ($stm->rowCount()>0){
                echo '<script>swal("  لقد تم ارسال التقييم لهذا الموظف اليوم    ", "لم يتم رفع البيانات", {
                    icon : "warning",
                    buttons: {        			
                        confirm: {
            ...
            elseif (!in_array($file_name_expload_end1, $exti)) {
                ...
                $query="INSERT INTO week_evaluation_form(sender_name,date,branch,file,emp)
                values (?,?,?,?,?) ";
                $stm=$con->prepare($query);
                $stm->execute(array($user,$today,$branch,$file_name1,$emp));
                if($stm->rowCount())
                {
                    move_uploaded_file($file_tmp1,"week_evaluation/".$file_name1);
