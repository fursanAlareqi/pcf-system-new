											...
											<div class="row">
												<div class="col-md-6 col-lg-4">
													...
													<div class="form-group form-floating-label">
														...
														<select class="selectpicker form-control" name="user" id="tokens" data-live-search="true" required>
															<option value=""></option>
															<?php
															// find branch
															...
															//end find branch 
															$sql = "select * from user where branch_id=? ";
															$stm = $con->prepare($sql);
															...
															?>
																	<option value="<?php echo $rows['id']; ?>" <?php if ($rows['id'] == $row['sender_name']) echo "selected"; ?>><?php echo $rows['name']; ?> </option>
...
<?php
	...
	if (isset($_POST['save'])) {
		...
		//اذا لايوجد اي مشكلة يقوم بارسال الملف
			...
			$query = "UPDATE  resption set code=?,name=?,brithday=?,date=?,
				...
				place=?,social_status=?,department=?,isolation=?,villag=?,briefness=?,
				  becouse_come=?,date_come=?,studing=?,identity=?,
				   identity_number=?,identity_place=?,laying=?,brother=?,work=?,
					...
					sister=?,know_our=?,sender_name=?,view=?,type=?,plasce_from_come=? where id=?
				 ";
			...
			if ($stm->rowCount()) {
				...
				echo "<script>
                                  window.open('admin_today_reception_data.php?num=5&id=3','_self')
                            </script>";
			} else {
