						...
						<div class="col-md-12">
								...
								<div class="card">
									<div class="card-header">
										<div class="card-title">نموذج   تقييم </div>
									</div>
									<div class="card-body">
                                                ...
                                                <div class="card-body">
                                                    <div class="row">
                                                        ...
                                                        <div class="col-7 col-stats">
                                                            <div class="numbers">
                                                                <h4 class="card-title">نموذج تقييم   </h4>
                                                            </div>
                                                        </div>
                                                    </div>
                                                </div>
...
$today=date("Y-m-d h:i:sa");
    ...
    if(isset($_POST['send'])){
        ...
        if ($stm->rowCount()>0){
            ...
            echo "<script> window.open('forms/نموذج تقييم اداء موظف.pdf','_self')</script>";
        }
        }
        else{
            echo '<script>swal("في حال عدم ارسال التقييم خلال 3 ايام فالمؤسسة تعتذرلك عن التقديم والقبول للنصف الثاني في مشروع مركز الدعم النفسي", "  ", {
                icon : "warning",
                buttons: {        			
                    confirm: {
                        className : "btn btn-warning"
            ...
            $query="INSERT INTO download_form(sender_name,branch,step,date)
                ...
                $stm->execute(array($user,$branch,'تقييم',$today));
                if($stm->rowCount())
                {
	...
	if(isset($_POST['save'])){
        ...
        $sender_name=$name['name'];
            ...
            $stm=$con->prepare($sql); 
            $stm->execute(array($user,'تقييم','False'));
	        if ($stm->rowCount()>0){
            ...
            elseif($hours>=72){
                ...
                values (?,?,?,?,?) ";
                $stm=$con->prepare($query);
                $stm->execute(array($user,$branch,$file_name1,'تقييم','False'));
                if($stm->rowCount())
                {
