							...
							<form action="" method="post">
								<div class="card">
									<div class="card-header">
										<div class="card-title">  التقرير اليومي  للاستشارة النقسية </div>
									</div>
									<div class="card-body">
										<div class="row">
	...
	function test_input($data){
	   ...
	   // $today= date('Y-m-d');
		...
		if($_SESSION['user']['rule_id']==16){
			$sql="select *  from hotline as res join consult_his_psychology as con on con.code=res.code  where res.type=? and res.sex=? and con.type=? and TIMESTAMPDIFF(YEAR,res.brithday,CURDATE()) between 1 and 17 and con.date=? and  con.sender_name=?";
		
		}
		else{
			$sql="select *  from resption as res join consult_his_psychology as con on con.code=res.code  where res.type=? and res.sex=? and con.type=? and TIMESTAMPDIFF(YEAR,res.brithday,CURDATE()) between 1 and 17 and con.date=? and  con.sender_name=?";
		...
		if($_SESSION['user']['rule_id']==16){
			$sql="select *  from hotline as res join consult_his_psychology as con on con.code=res.code  where res.type=? and res.sex=? and con.type=? and TIMESTAMPDIFF(YEAR,res.brithday,CURDATE())  >= 18 and con.date=? and  con.sender_name=? ";
		
		}
		else{
			$sql="select *  from resption as res join consult_his_psychology as con on con.code=res.code  where res.type=? and res.sex=? and con.type=? and TIMESTAMPDIFF(YEAR,res.brithday,CURDATE())  >= 18 and con.date=? and  con.sender_name=? ";
