...
if(!isset($_SESSION['user'])){
 ...
 }if($_SESSION['user']['rule_id']==11 ||$_SESSION['user']['rule_id']==13 ||$_SESSION['user']['rule_id']==2 ||$_SESSION['user']['rule_id']==6){
?>
							...
							<form action="" method="post">
                                ...
                                <div class="col-md-6 col-lg-4">
                                        <div class="form-group form-floating-label">
                                            ...
                                            <label for="tokens">اسم العلاج </label>
                                            <select class="selectpicker form-control" name="medicine_name"  id="tokens"  data-live-search="true" required >
                                                <option value=""></option>
                                                <?php 
                                                    $branch=$_SESSION['user']['branch_id'];
                                                    $sql="select * from pharmacy where branch=?  and date > ? ORDER BY `pharmacy`.`name` ASC  ";
                                                    $stm=$con->prepare($sql); 
                                                ...
                                                ?>
                                                    ...
                                                    <option value="<?php echo $row['id']; ?>">exporter:<?php echo $row['exporter'];?> & <?php echo $row['name'];?>&  ^<?php echo $row['residual']; ?> <br> تاريخ الادخال <?php echo $row['date']; ?></option>
								...
								<?php if(isset($_POST['search'])){
										...
										$sql="SELECT * FROM pure_pharmacy where name like ? and date BETWEEN ? and ? and branch=?   ";	
										$stmu=$con->prepare($sql); 
										$stmu->execute(array("%$medicine_name%",$from_date,$to_date,$branch));
										//عدد الحالات الكلية
										if($stmu->rowCount()>0){
