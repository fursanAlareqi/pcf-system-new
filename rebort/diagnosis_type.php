                            ...
                            for($j=0;$j<sizeof($sex);$j++){
                                ...
                                echo "<td>";
                                    if($n==1){
                                        ...
                                        $sql="SELECT * from resption as res join psyshological as psy on (res.code=psy.code) where (TIMESTAMPDIFF(YEAR,res.brithday,CURDATE()) <= ?) and (res.type=?) and (psy.diagnosis = ?) and  (res.sex=? ) and (res.project like ?) and psy.date between ? and ?and (res.branch=?) ";
                                        $stm=$con->prepare($sql); 
                                        $stm->execute(array($age,'جديد',$diagnosis_type[$i],$sex[$j],"%$project%",$from_date,$to_date,$branch ));
                        ...
                        elseif($n==2||$n==3){
                            ...
                            for($j=0;$j<sizeof($sex);$j++){
                                ...
                                echo "<td>";
                                    ...
                                    $sql="SELECT * from resption as res join psyshological as psy on (res.code=psy.code) where (TIMESTAMPDIFF(YEAR,res.brithday,CURDATE())  between ? and ?) and (res.type=?) and (psy.diagnosis = ?) and  (res.sex=? ) and (res.project like ?) and psy.date between ? and ? and (res.branch=?) ";
                                    $stm=$con->prepare($sql); 
                                    $stm->execute(array($from_age,$to_age,'جديد',$diagnosis_type[$i],$sex[$j],"%$project%",$from_date,$to_date,$branch));
                                    $total=$total+ $stm->rowCount();
                                    echo $stm->rowCount();
                                echo "</td>";
