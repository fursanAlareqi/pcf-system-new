...
if(isset($_POST['save'])){
	...
	if(	empty($_POST["violense"])){
		...
		{
			$sql="select * from psyshological where  code=? and date=?  ";
			$stm=$con->prepare($sql); 
			...
			if($stm->rowCount()){
				...
				Suicide_Homicide,Speech,Thinking,Perception,Insight,sender_name,lap,sub_diagnosis,appraisal,type,quantity)
				values (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?) ";
				...
				$Thinking,$Perception,$Insight,$user,$b,$sub_diagnosis,$appraisal,'عودة',$quantity));
				if($stm->rowCount())	
				{
