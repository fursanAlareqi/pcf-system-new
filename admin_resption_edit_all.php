													...
													<div class="form-group form-floating-label">
														...
														<label> الفرع</label>
															<?php
																...
																?>
														<input type="data" value="<?php echo $branch_name; ?>" disabled class="form-control input-border-bottom" id="inputFloatingLabel" required>

														<input type="hidden" value="<?php echo $row['branch']; ?>" class="form-control input-border-bottom" id="inputFloatingLabel" name="branch" required>
...
<?php
	...
	if (isset($_POST['save']) ) {
		...
		//اذا لايوجد اي مشكلة يقوم بارسال الملف
			...
			$query = "UPDATE  resption set sex=?,name=?,
				  brithday=? Where code=? and branch =?
				 ";
			...
			if ($stm->rowCount()) {
				...
				echo "<script>
                                  window.open('admin_today_reception_data.php?num=3&id=119','_self')
                            </script>";
			} else {
