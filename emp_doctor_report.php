...
if(isset($_POST['save'])){
	function test_input($data){
	   ...
	   // $today= date('Y-m-d');
		...
		$sql="select *  from resption as res join doctor as con on con.code=res.code  where res.type=? and res.sex=? and con.type=? and TIMESTAMPDIFF(YEAR,res.brithday,CURDATE()) between 1 and 17 and con.date=? and  con.sender_name=?";
		$stm=$con->prepare($sql); 
		...
		$sql="select *  from resption as res join doctor as con on con.code=res.code  where res.type=? and res.sex=? and con.type=? and TIMESTAMPDIFF(YEAR,res.brithday,CURDATE())  >= 18 and con.date=? and  con.sender_name=? ";
		$stm=$con->prepare($sql); 
		$stm->execute(array('جديد','ذكر','جديد',$date,$user));
