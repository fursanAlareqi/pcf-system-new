							...
							<form action="" method="post">
								<div class="card">
									<div class="card-header">
										<div class="card-title"> التقرير اليومي للجلسات الجماعية</div>
									</div>
									<div class="card-body">
										<div class="row">
	...
	if(isset($_POST['save'])){
		function test_input($data){
		   ...
		   // $today= date('Y-m-d');
			...
			$sql="select code  from group_sessions where  date=? and sender_name=?";
			...
			if($stmu->rowCount()){
				...
				foreach($all_code_explode as $code ){
					if($_SESSION['user']['rule_id']==16){
						$sql="select * from hotline where code=? and  sex=? and type=? and TIMESTAMPDIFF(YEAR,brithday,CURDATE()) between 1 and 17 and date=?";
					
					}
					else{
						$sql="select * from resption where code=? and  sex=? and type=? and TIMESTAMPDIFF(YEAR,brithday,CURDATE()) between 1 and 17 and date=?";
					...
					if($_SESSION['user']['rule_id']==16){
						$sql="select * from hotline where code=? and  sex=? and type=? and TIMESTAMPDIFF(YEAR,brithday,CURDATE()) >= 18 and date=?";
					
					}
					else{
						$sql="select * from resption where code=? and  sex=? and type=? and TIMESTAMPDIFF(YEAR,brithday,CURDATE()) >= 18 and date=?";
