								...
								<form action="" method="post">
									<div class="card">
										<div class="card-header">
											<div class="card-title"> تعديل بيانات ادارة الحالة</div>
										</div>
										<div class="card-body">
											<div class="row">
...
<?php
	...
	if (isset($_POST['save'])) {
		...
		//اذا لايوجد اي مشكلة يقوم بارسال الملف
			...
			$query = "UPDATE mange_convert SET branch=?,type_con_from=?,side_con_from=?,type_con_to=?,side_con_to=?,project_name=?,violense=?,date=?,type_con_out=?,side_con_out=?,type_con_come_out=?,side_con_come_out=?,find=?,person_name=?,person_phone=? where id=? ";
			$stm = $con->prepare($query);
			$stm->execute(array($branch, $type_con_from, $side_con_from, $type_con_to, $side_con_to, $project_name, $b, $date, $type_con_out, $side_con_out, $type_con_come_out, $side_con_come_out, $find, $person_name, $person_phone, $id));
			if ($stm->rowCount()) {
				echo '<script src="js/send_success.js"></script>';
				echo "<script>
                                  window.open('today_convert_data.php?num=5&id=5','_self')
                         </script>";
			} else {
