						...
						<div class="col-md-12">
									...
									<div class="form-group form-floating-label">
										...
										<select class="selectpicker form-control" name="code" id="tokens"  data-live-search="true" >
											...
											<?php 
												...
												//end find branch 
												$sql="select DISTINCT(code) from sleeping_afternoon where branch=? ";
												$stm=$con->prepare($sql); 
							...
							<input class="btn btn-success" type="submit" name="search" value="search">
								...
								<?php if(isset($_POST['search'])){
										...
										$sql="SELECT * FROM sleeping_afternoon where code=?  ORDER BY `sleeping_afternoon`.`date` desc  ";	
...
if(isset($_POST['save'])){
	function test_input($data) {
		...
		{
			$sql="select * from sleeping_afternoon where  code=? and date=?  ";
			$stm=$con->prepare($sql); 
			...
			if($stm->rowCount()){
				...
				$query="INSERT INTO sleeping_afternoon(branch,code,sender_name,date,exit_date,visites,medicin_m,hart_m,heat_m,breathe_m,inside_m,outside_m,medical,program,type)
				values (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?) ";
				$stm=$con->prepare($query);
				$stm->execute(array($branch,$code,$user,$date,$exit_date,$visites,$medicin_m,$hart_m,$heat_m,$breathe_m,$inside_m,$outside_m,$medical,$program,'عودة'));
				if($stm->rowCount())	
				{
