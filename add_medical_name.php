...
if(!isset($_SESSION['user'])){
 ...
 }if($_SESSION['user']['rule_id']==11 ||$_SESSION['user']['rule_id']==6 ||$_SESSION['user']['rule_id']==2){
?>
							...
							<form action="" method="post">
								<div class="card">
									<div class="card-header">
										<div class="card-title"> اضافة  صنف  </div>
									</div>
									<div class="card-body">
										<div class="row">
										...
										<table id="multi-filter-select" class="table table-bordered table-head-bg-info table-bordered-bd-info mt-4" >
											...
											<tbody>
												<?php
													...
													$sql="select * from scientific_pharmacy_name where branch =? ";
													$stm=$con->prepare($sql); 
...
//تم وضع كود في الاخير من اجل تنفيذ رسالة التحدذير
	if(isset($_POST['save'])){
		...
		$branch=$_SESSION['user']['branch_id'];
			...
			{
				$sql="select * from scientific_pharmacy_name where  name=?";
				$stm=$con->prepare($sql); 
				...
				if($stm->rowCount()){
					...
					$query="INSERT INTO scientific_pharmacy_name(name,branch,sender_name)
					values (?,?,?) ";
					$stm=$con->prepare($query);
					$stm->execute(array($specialty,$branch,$user));
					if($stm->rowCount())
					{
