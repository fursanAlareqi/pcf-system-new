	...
	if(isset($_POST['save'])){
		function test_input($data){
		   ...
		   // $today= date('Y-m-d');
			...
			$sql="select * from resption where  sex=? and type=? and TIMESTAMPDIFF(YEAR,brithday,CURDATE()) between 1 and 17 and date=? and sender_name=?";
			$stm=$con->prepare($sql); 
			$stm->execute(array('ذكر','جديد',$date,$user));
			...
			$sql="select * from resption where  sex=? and type=? and TIMESTAMPDIFF(YEAR,brithday,CURDATE()) between 1 and 17 and date=? and sender_name=? ";
			$stm=$con->prepare($sql); 
			$stm->execute(array('ذكر','عودة',$date,$user));
			...
			$sql="select * from resption where  sex=? and type=? and TIMESTAMPDIFF(YEAR,brithday,CURDATE()) >= 18 and date=? and sender_name=?";
			$stm=$con->prepare($sql); 
			$stm->execute(array('ذكر','جديد',$date,$user));
			...
			$sql="select * from resption where  sex=? and type=? and TIMESTAMPDIFF(YEAR,brithday,CURDATE()) between 1 and 17 and date=? and sender_name=?";
			$stm=$con->prepare($sql); 
			$stm->execute(array('انثى','جديد',$date,$user));
			...
			$sql="select * from resption where  sex=? and type=? and TIMESTAMPDIFF(YEAR,brithday,CURDATE()) between 1 and 17 and date=? and sender_name=?";
			$stm=$con->prepare($sql); 
			$stm->execute(array('انثى','عودة',$date,$user));
