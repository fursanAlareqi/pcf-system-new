						...
						<div class="col-md-12">
								...
								<div class="card">
									<div class="card-header">
										<div class="card-title">نموذج اخلاء طرف</div>
									</div>
									<div class="card-body">
                                                    ...
                                                    <div class="card-body">
                                                        <div class="row">
                                                            ...
                                                            <div class="col-7 col-stats">
                                                                <div class="numbers">
                                                                    <h4 class="card-title">نموذج اخلاء طرف</h4>
                                                                </div>
                                                            </div>
                                                        </div>
                                                    </div>
                    ...
                    <div class="row">
						...
						<div class="col-md-12">
                            <form action="" method="post" enctype="multipart/form-data">
                                <div class="card">
                                    <div class="card-header">
                                        <div class="card-title">  رفع اخلاء طرف</div>
                                    </div>
                                    <div class="card-body">
                                        <div class="row">
                                            ...
                                            <div class="col-md-6 col-lg-4">
                                                ...
                                                <div class="form-group">
                                                    <label for="exampleFormControlFile1">رفع اخلاء طرف</label>
...
$today=date("Y-m-d h:i:sa");
    ...
    if(isset($_POST['send'])){
        ...
        $stm=$con->prepare($sql); 
        $stm->execute(array($user,'اخلاء طرف'));
        ...
        if ($stm->rowCount()>0){
            ...
            echo "<script> window.open('forms/اخلاء طرف نموذج.pdf','_self')</script>";
        }
        }
        else{
            echo '<script>swal("في حال عدم ارسال اخلاء طرف خلال 3 ايام فالمؤسسة تعتذرلك عن التقديم والقبول للنصف الثاني في مشروع مركز الدعم النفسي", "  ", {
                icon : "warning",
                buttons: {        			
                    confirm: {
                        className : "btn btn-warning"
            ...
            $query="INSERT INTO download_form(sender_name,branch,step,date)
                ...
                $stm->execute(array($user,$branch,'اخلاء طرف',$today));
                if($stm->rowCount())
                {
	...
	if(isset($_POST['save'])){
        ...
        $sender_name=$name['name'];
            ...
            $stmd->execute(array($user,'اخلاء طرف'));
            ...
            $stm=$con->prepare($sql); 
            $stm->execute(array($user,'اخلاء طرف','False'));
	        if ($stm->rowCount()>0){
            ...
            elseif($hours>=72){
                ...
                values (?,?,?,?,?) ";
                $stm=$con->prepare($query);
                $stm->execute(array($user,$branch,$file_name1,'اخلاء طرف','False'));
                if($stm->rowCount())
                {
