						...
						<div class="col-md-12">
							...
							<input class="btn btn-success" type="submit" name="search" value="search">
								...
								<?php if(isset($_POST['search'])){
										...
										$sql="SELECT * FROM sleeping_morning where code=?  ORDER BY `sleeping_morning`.`date` desc  ";	
...
if(isset($_POST['save'])){
	function test_input($data) {
		...
		{
			$sql="select * from sleeping_morning where  code=? and date=?  ";
			$stm=$con->prepare($sql); 
			...
			if($stm->rowCount()){
				...
				$query="INSERT INTO sleeping_morning(branch,code,sender_name,date,exit_date,visites,medicin_m,hart_m,heat_m,breathe_m,inside_m,outside_m,medical,program,type)
				values (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?) ";
